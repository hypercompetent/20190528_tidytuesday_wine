...
~52k different words - that's a decent set of features.

Let's build a sparse matrix of the occurences of these words for each description.
```{r}
library(Matrix)
...
That's pretty slow, so I'll save the matrix for later:
```{r}
...
word_occurences <- Matrix::colSums(t_binary_matrix)
occurence_threshold <- nrow(t_binary_matrix) * 0.001

ncounts <- t(t_binary_matrix[,word_occurences >= occurence_threshold])
